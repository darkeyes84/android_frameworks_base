...
public abstract class Ticker {
    ...
    public void setIconColorTint(int color) {
        mIconTint = ColorStateList.valueOf(color);
        if (mSegments.size() > 0) {
            Segment seg = mSegments.get(0);
            seg.icon.setColorFilter(null);
            seg.icon.setColorFilter(color, PorterDuff.Mode.SRC_ATOP);
		}
    }

    public void setTextColor(int color) {
        mTextColor = color;
        mTextSwitcher.setTextColor(color);
    }
}
