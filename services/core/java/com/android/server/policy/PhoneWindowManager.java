...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private boolean layoutNavigationBar(int displayWidth, int displayHeight, int displayRotation,
        ...
        if (mNavigationBar != null) {
            ...
            if (mNavigationBarPosition == NAV_BAR_BOTTOM) {
                ...
                int top = (immersiveModeImplementsPie() ? displayHeight - overscanBottom : displayHeight - overscanBottom
                        - getNavigationBarHeight(displayRotation, uiMode));
                mTmpNavigationFrame.set(0, top, displayWidth, displayHeight - overscanBottom);
                mStableBottom = mStableFullscreenBottom = mTmpNavigationFrame.top;
                if (transientNavBarShowing) {
                    mNavigationBarController.setBarShowingLw(!immersiveModeImplementsPie());
                } else if (navVisible) {
                    mNavigationBarController.setBarShowingLw(!immersiveModeImplementsPie());
                ...
                int right = (immersiveModeImplementsPie() ? overscanLeft 
                        : overscanLeft + getNavigationBarWidth(displayRotation, uiMode));
                ...
                int left = (immersiveModeImplementsPie() ? displayWidth - overscanRight
                        : displayWidth - overscanRight - getNavigationBarWidth(displayRotation, uiMode));
    ...
    private int updateSystemUiVisibilityLw() {
        ...
        if (wasCleared && !mClearedBecauseOfForceShow
                && (tmpVisibility & View.SYSTEM_UI_FLAG_HIDE_NAVIGATION) != 0 && !immersiveModeImplementsPie()) {
            mNavigationBarController.showTransient();
